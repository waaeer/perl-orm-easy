...
$dbh->do(q!CREATE USER tester!);
$dbh->do(q!GRANT USAGE ON SCHEMA orm_interface TO tester!);
$dbh->do(q!GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA orm_interface TO tester!);

my $dbt = DBI->connect($pgsql->dsn, 'tester', undef, {RaiseError => 1});

foreach my $i (1..100) {
	$dbt->do(qq!SELECT orm_interface.save('public','object', NULL, 0, '{"id":$i, "name": "x$i", "x": $i}', '{}')!);
}
...
$dbt->do(qq!SELECT orm_interface.msave('public','object', 0, 1, 2, '{"_order":"id"}', '{"name":"xyz"}', '{}')!);

is(normalize_json( $dbt->selectcol_arrayref(qq!SELECT orm_interface.mget('public', 'object', 0, 1, 4, '{"_order":"id","_fields":["id","name"]}')!)->[0]),
...
$dbt->do(qq!SELECT orm_interface.save('public','test_table', NULL, 0, '{"id":1, "c": [ "2018-02-01", null ]}', '{}')!);
$dbt->do(qq!SELECT orm_interface.save('public','test_table', NULL, 0, '{"id":2, "c": [ null, "2018-02-01" ]}', '{}')!);
$dbt->do(qq!SELECT orm_interface.save('public','test_table', NULL, 0, '{"id":3, "c": [ null, "2018-02-01" , "(]" ]}', '{}')!);
...
$dbh->do(q!GRANT USAGE ON SCHEMA schema1 TO tester!);
$dbh->do(q!GRANT USAGE ON SCHEMA schema2 TO tester!);
...
$dbh->do(q!GRANT INSERT ON schema1.first_subclass TO tester!);
$dbh->do(q!GRANT SELECT ON schema1.first_subclass TO tester!);
$dbh->do(q!GRANT DELETE ON schema1.first_subclass TO tester!);
$dbh->do(q!GRANT INSERT ON schema2.some_referencing_class TO tester!);


$dbt->do(qq!INSERT INTO schema1.first_subclass VALUES (1,'XXX'),(2,'YYY'),(3,'ZZZ')!);
$dbt->do(qq!INSERT INTO schema2.some_referencing_class VALUES (4,2)!);

eval {
	$dbt->do(qq!INSERT INTO schema2.some_referencing_class VALUES (5,5)!);
};
is(dbt_error(), "schema2.some_referencing_class.some_ref = 5 should reference to schema1.some_base_class.id", 'abstract FK insert');

my $n = $dbh->selectcol_arrayref(qq!SELECT count(*) FROM schema2.some_referencing_class!)->[0];
is($n,1, 'abstract FK n');

eval {
  $dbt->do(qq!DELETE FROM schema1.first_subclass WHERE id=2!);
};
is(dbt_error(), "Referenced object deletion: schema1.some_base_class.id = schema2.some_referencing_class.some_ref", 'abstract FK delete');
...
$dbh->do(q!GRANT INSERT ON schema1.t1 TO tester!);
$dbh->do(q!GRANT SELECT ON schema1.t1 TO tester!);
$dbh->do(q!GRANT DELETE ON schema1.t1 TO tester!);
$dbh->do(q!GRANT INSERT ON schema2.t2 TO tester!);

$dbt->do(qq!INSERT INTO schema1.t1 VALUES (1), (2), (3)!);

$dbt->do(qq!INSERT INTO schema2.t2 VALUES (4, ARRAY[1,2], ARRAY[1])!);
eval {
	$dbt->do(qq!INSERT INTO schema2.t2 VALUES (5, ARRAY[5], ARRAY[1])!);
};
is(dbt_error(), "schema2.t2.refs = 5 should reference schema1.t1.id", 'array FK insert');
eval {
	$dbt->do(qq!INSERT INTO schema2.t2 VALUES (6, ARRAY[1,NULL])!);
};
is(dbt_error(), "schema2.t2.refs =  should reference schema1.t1.id", 'array FK insert 2');
eval {
	$dbt->do(qq!DELETE FROM schema1.t1 WHERE id = 1!);
};
is(dbt_error(), "Referenced object deletion: schema1.t1.id in schema2.t2.refs", 'array FK delete');
